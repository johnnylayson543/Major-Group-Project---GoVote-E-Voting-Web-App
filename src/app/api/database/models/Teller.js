...
class TellerClass {
    ...
    static async retrieve_the_labels_for_the_chart(x) {
        try {
            const tally = await Tally.findOne({ electionID: x.electionID });
            console.log("tally:");
            console.log(tally);
            if (tally != {}) {
                const labels = 
                    tally.tally.map(async ({ candidateID }) => {
                        const candidate = await Candidate.findOne({ _id: candidateID });
                        const { ppsn, name } = await Person.findOne({ ppsn: candidate.person_ppsn });
                        return `${ppsn} | ${name}`;
                    })
                ;
            ...
            console.error('Error retrieving the labels: ', error);
